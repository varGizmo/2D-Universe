...
function callback_function(json) {
	switch(mode){
		...
		case "both":
			let directory=destination.replace(/\/+$/,'');//strip last path slashes
...
function processClient(json, dest){
	let jsonMap = JSON.stringify(processMap(json, {mode:"client"})); // Save the processed map object as JSON data
...
function processServer(json, dest){
	let jsonMap = JSON.stringify(processMap(json, {mode:"server"})); // Save the processed map object as JSON data
