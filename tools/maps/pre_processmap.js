...
function preprocessMap(json) {
    if (json.tilesets instanceof Array) {
        _.each(json.tilesets, function(tileset) {
            let tileSetWidth = tileset.imagewidth / tileset.tilewidth;
            if(!isInt(tileSetWidth)){
				log.error("** The tileSetWidth ("+tileSetWidth+") is not an integer… abort! abort!");
				return false;
			}
			let c=[], currentOrigin=0, tileId, countBase=0; //Origin / Base pointers to fifo C
...
function save(json, dest){
	if(json !== false){
		let jsonMap = JSON.stringify(json); // Save the processed map object as JSON data
