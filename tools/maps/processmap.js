...
let log = new Log(Log.DEBUG, fs.createWriteStream('processmap.log'));
...
module.exports = function processMap(json, options) {
    let self = this, TiledJSON = json;
    let layerIndex = 0, tileIndex = 0;
    ...
    let handleTileProp = function(propName, propValue, tileId) {
        if(propName === "c") {
    ...
    if (TiledJSON.tilesets instanceof Array) {
        _.each(TiledJSON.tilesets, function(tileset) {
            let tileSetName = tileset.name.toLowerCase();
    ...
    _.each(TiledJSON.layers, function(layer) {
        ...
        if (layerName === "doors" && layerType === "objectgroup") {
            ...
            for (let j = 0; j < doors.length; j += 1) {
                ...
                let doorProperties = doors[j].properties;
        ...
        // Roaming Mob Areas
            ...
            for (let j = 0; j < areas.length; j += 1) {
                if(areas[j].properties) {
                    let nb = parseInt(areas[j].properties.nb, 10);
                }
    ...
    for(let i = TiledJSON.layers.length - 1; i > 0; i -= 1) {
...
let processLayer = function(layer) {
    ...
    if (mode === "server" && layerName === "entities") {
        ...
        for(let j = 0; j < tiles.length; j += 1) {
            let gid = tiles[j] - mobsFirstGid + 1;
            if(gid && gid > 0) {
