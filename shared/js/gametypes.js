...
Types.getWeaponRank = function (weaponKind, _) {
    return _.indexOf(Types.rankedWeapons, weaponKind);
};
Types.getArmorRank = function (armorKind, _) {
...
Types.forEachKind = function (cb) {
    for (let k in kinds) {
        cb(kinds[k][0], k);
    }
};
Types.forEachArmor = function (cb) {
...
Types.forEachMobOrNPCKind = function (cb) {
    Types.forEachKind(function (kind, kindName) {
...
Types.forEachArmorKind = function (cb) {
    Types.forEachKind(function (kind, kindName) {
        if (Types.isArmor(kind)) {
            cb(kind, kindName);
        }
...
Types.forEachWeaponKind = function (cb) {
    Types.forEachKind(function (kind, kindName) {
...
Types.getRandomItemKind = function (item, _) {
    let all = _.union(Types.rankedWeapons, Types.rankedArmors), forbidden = [Types.Entities.SWORD1, Types.Entities.CLOTHARMOR], itemKinds = _.difference(all, forbidden), i = Math.floor(Math.random() * _.size(itemKinds));
    return itemKinds[i];
};
Types.getMessageTypeAsString = function (type, _) {
    let typeName;
